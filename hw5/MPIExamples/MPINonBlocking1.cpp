...
int main(int argc, char**argv)
{
    ...
    for (int round = 0; round < 100; ++round)
    {
        ...
        { // Rank zero sends first then receives, all other 
          // receive first then send
            cout << "Rank " << rank
                ...
                << " round " << round << endl;
            MPI_Request request;
            rc = MPI_Isend(buf, sizeof(buf), MPI_CHAR, rank + 1, 0, MPI_COMM_WORLD, &request);
            ...
            rc = MPI_Irecv(buf, sizeof(buf), MPI_CHAR, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &request);
            if (rc != MPI_SUCCESS)
            {
                cout << "Rank " << rank
        ...
        else
        {
            ...
            rc = MPI_Send(buf, sizeof(buf), MPI_CHAR, nextRank, 0, MPI_COMM_WORLD);
